...
class MarketAnalysis {
    ...
    def processAllMarketAnalytics(spark: SparkSession, businessDF: DataFrame, reviewsDF: DataFrame): Unit = {
        ...
        println(s"7- Market rating distribution: ${ratingDist.count()} distributions")
        
        println(" ================ Toutes les analyses de marché terminées et sauvegardées! ================ ")
    }

    def integrateMarketAnalysis(spark: SparkSession, businessDF: DataFrame, reviewsDF: DataFrame): Unit = {
